...
// Reset all usage counters while preserving quality scores
export function resetUsageCounters(): void {
  // Keep quality scores but reset usage counters
  Object.keys(keyUsageData).forEach(key => {
    const quality = keyUsageData[key].qualityScore;
    keyUsageData[key] = {
      useCount: 0,
      tokenCount: 0, 
      successCount: 0,
