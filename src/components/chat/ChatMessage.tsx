...
export const ChatMessage: React.FC<ChatMessageProps> = ({
  ...
  if ((!content || content.trim() === '') && sender === 'bot' && !isTranslating && !translationInProgress && isError) {
    return <div className="flex justify-start mb-4 w-full">
        <Card className="p-3 rounded-lg bg-red-50 text-red-800 border-red-200 dark:bg-red-900/30 dark:text-red-300 w-full">
          <div className="whitespace-pre-line">
            ...
            {onRetry && <div className="mt-2">
                <Button variant="outline" size="sm" onClick={onRetry} className="bg-red-100 text-red-800 border-red-200 hover:bg-red-200 dark:bg-red-900/50 dark:text-red-300 dark:border-red-700">
  ...
  return <div className={`flex ${sender === 'user' ? 'justify-end' : 'justify-start'} mb-4 w-full`}>
      <div className={`flex items-start gap-3 w-full ${sender === 'user' ? 'flex-row-reverse' : ''}`}>
        <Card className={`p-3 rounded-lg w-full ${sender === 'user' ? 'bg-finance-medium-blue text-white' : isError ? 'bg-red-50 text-red-800 border-red-200 dark:bg-red-900/30 dark:text-red-300' : translationInProgress ? 'bg-gray-50 dark:bg-gray-800 opacity-70' : 'bg-gray-50 dark:bg-gray-800'}`}>
          {/* Bot message content */}
          {sender === 'bot' && !isTypingComplete && !isTranslating && !translationInProgress && <TypingAnimation text={displayContent} className="whitespace-pre-line text-left" onComplete={() => setIsTypingComplete(true)} onProgress={onTypingProgress} />}
          
          {/* User message content or bot message when translation is in progress */}
          {(sender === 'user' || isTranslating || translationInProgress || sender === 'bot' && isTypingComplete) && <div className="">
