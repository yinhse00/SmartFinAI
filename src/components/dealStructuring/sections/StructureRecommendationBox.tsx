...
import { ConsiderationCalculationResult } from '@/services/dealStructuring/considerationCalculator';
interface StructureRecommendationBoxProps {
  results: AnalysisResults;
  userInputs?: ExtractedUserInputs & { calculationResult?: ConsiderationCalculationResult };
  description?: string;
}
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="bg-white rounded p-2 border">
                  ...
                  <div className="font-semibold text-green-600">
                    {processedResults._calculatedConsideration ? (
                      <div>
                        <div>{userInputs?.currency || 'HKD'} {Math.round(processedResults._calculatedConsideration * (processedResults.structure.majorTerms.paymentStructure.cashPercentage / 100)).toLocaleString()}</div>
                        <div className="text-xs text-gray-500">({processedResults.structure.majorTerms.paymentStructure.cashPercentage}%)</div>
                      </div>
                    ) : (
                ...
                <div className="bg-white rounded p-2 border">
                  ...
                  <div className="font-semibold text-green-600">
                    {processedResults._calculatedConsideration ? (
                      <div>
                        <div>{userInputs?.currency || 'HKD'} {Math.round(processedResults._calculatedConsideration * (processedResults.structure.majorTerms.paymentStructure.stockPercentage / 100)).toLocaleString()}</div>
                        <div className="text-xs text-gray-500">({processedResults.structure.majorTerms.paymentStructure.stockPercentage}%)</div>
                      </div>
                    ) : (
              ...
              <div>
                ...
                <div className="text-xs text-green-700 bg-green-50 rounded p-2 border border-green-200">
                  <div className="font-semibold mb-1">
                    Total Consideration: {userInputs?.currency || 'HKD'} {processedResults._calculatedConsideration.toLocaleString()}
                  </div>
                  {userInputs?.calculationResult?.calculationDetails?.calculationFormula && (
                    <div className="text-gray-600 text-xs mt-1">
                      Formula: {userInputs.calculationResult.calculationDetails.calculationFormula}
                    </div>
                  )}
                  {userInputs?.calculationResult?.calculationMethod === 'calculated' && (
                    <div className="text-green-600 text-xs mt-1 font-medium">
                      ✓ Calculated from target valuation and acquisition percentage
                    </div>
                  )}
                </div>
              </div>
